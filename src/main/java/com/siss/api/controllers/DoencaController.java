...
public class DoencaController {
	...
	@PreAuthorize("hasAnyRole('USUARIO')")
	@PostMapping
	public ResponseEntity<Response<DoencaDto>> salvar(@Valid @RequestBody DoencaDto doencaDto, BindingResult result) {
		Response<DoencaDto> response = new Response<DoencaDto>();
		try {
	...
	public ResponseEntity<String> excluirPorId(@PathVariable("id") int id) {
		try {
			log.info("Controller: excluíndo doenca de ID: {}", id);
			doencaService.excluirPorId(id);
			return ResponseEntity.ok("doenca de id: " + id + " excluído com sucesso");
		} catch (ConsistenciaException e) {
			log.info("Controller: Inconsistência de dados: {}", e.getMessage());
